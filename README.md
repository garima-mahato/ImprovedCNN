...
Receptive Field is the area at which the filter is looking in the input matrix. Local receptive field is the area in the immediate input matrix,i.e. the input to the current layer(to which the filter belongs) of the network, at which the filter is looking while global receptive field is the area in the initial layer's input matrix to the network at which the filter is looking.
